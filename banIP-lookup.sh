...
for feed in ${feeds}; do
	...
	while IFS= read -r domain; do
		(
			out="$("${dig_tool}" "@${upstream1}" "${domain}" A "${domain}" AAAA +noall +answer +time=5 +tries=3 2>/dev/null)"
			if [ -n "${out}" ]; then
				ips="$(printf "%s" "${out}" | "${awk_tool}" '/^.*[[:space:]]+IN[[:space:]]+A{1,4}[[:space:]]+/{printf "%s ",$NF}' 2>/dev/null)"
				if [ -n "${ips}" ]; then
					for ip in ${ips}; do
	...
	while IFS= read -r domain; do
		(
			out="$("${dig_tool}" "@${upstream2}" "${domain}" A "${domain}" AAAA +noall +answer +time=5 +tries=3 2>/dev/null)"
			if [ -n "${out}" ]; then
				ips="$(printf "%s" "${out}" | "${awk_tool}" '/^.*[[:space:]]+IN[[:space:]]+A{1,4}[[:space:]]+/{printf "%s ",$NF}' 2>/dev/null)"
				if [ -n "${ips}" ]; then
